<template>
  <div class="page">
    ...
    <form @submit.prevent="submit">
      <label>
         转出账户：
         <input v-model="form.fromAccount" required />
      </label>
      <label>
         转入账户：
         <input v-model="form.toAccount" required />
      </label>
      <label>
        金额：
...
const form = ref({ fromAccount: '', toAccount: '', amount: null })
...
const submit = async () => {
  ...
  error.value = ''
  try {
    const payload = { ...form.value, txId: `TRANSFER_${Date.now()}_${Math.random().toString(36).slice(2,8)}` }
